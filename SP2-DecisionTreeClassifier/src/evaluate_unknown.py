...
def predict_with_threshold(model, X, threshold):
    ...
    raw_pred = model.classes_[class_indices]
...
def main():
    ...
    parser.add_argument("--model-path", default="models/decision_tree_tuned.joblib")
    parser.add_argument("--output-path", default="reports/unknown_threshold_results.csv")
    ...
    y_eval = pd.concat([y_known_test.reset_index(drop=True), y_unknown], ignore_index=True)
    ...
    print("========== UNKNOWN THRESHOLD RESULTS ==========")
    print(result_df.to_string(index=False))
    print(f"\nSaved to: {args.output_path}")

    # Print one detailed report at a reasonable default threshold.
    threshold = 0.60
    y_pred, _, _ = predict_with_threshold(model, X_eval, threshold)
